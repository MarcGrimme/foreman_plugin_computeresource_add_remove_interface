module ForemanPluginComputeresourceAddRemoveInterface
  class MyunattendedController < ApplicationController
    layout nil
  ...
  def get_host_details
    logger.info "myunattended_controller called"
  ...
  def find_host_by_ip_or_mac
    ...
    if request.env["action_dispatch.request.query_parameters"]['HTTP_X_FORWARDED_FOR'].present? and (ip =~ Regexp.new(Setting[:remote_addr]))
      ip = request.env["action_dispatch.request.query_parameters"]['HTTP_X_FORWARDED_FOR']
    end
    ...
    mac_list = []
    if request.env["action_dispatch.request.query_parameters"]['HTTP_X_RHN_PROVISIONING_MAC_0'].present?
      begin
